...
DROP TRIGGER IF EXISTS award_achievement_badges ON public.achievements;
DROP TRIGGER IF EXISTS award_points_badges_trigger ON public.athletes;
DROP TRIGGER IF EXISTS award_category_badges_trigger ON public.category_history;

DROP FUNCTION IF EXISTS public.check_and_award_badges();
DROP FUNCTION IF EXISTS public.award_points_badges();
DROP FUNCTION IF EXISTS public.award_category_badges();

DROP TABLE IF EXISTS public.athlete_badges CASCADE;
...
-- Function to check and award badges based on tournament achievements
...
DECLARE
  ...
  has_bronze boolean;
BEGIN
  -- Get athlete tournament stats
  SELECT 
    COUNT(*) FILTER (WHERE position = 1),
    COUNT(*) FILTER (WHERE position = 2),
    COUNT(*) FILTER (WHERE position = 3),
    COUNT(*),
    COUNT(*) FILTER (WHERE position = 1) > 0,
    COUNT(*) FILTER (WHERE position = 2) > 0,
    COUNT(*) FILTER (WHERE position = 3) > 0
  INTO 
    first_place_count,
    second_place_count,
    third_place_count,
